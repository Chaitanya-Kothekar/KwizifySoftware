...
const DropdownClass = ({setClass }) => {
  ...
  return (
    <div className="relative inline-block text-center shadow rounded-md" ref={dropdownRef}>
      <button
        ...
        >
          {classData.length > 0 ? (
            classData.map((classdata) => (
              <button
                onClick={() => handleOptionClick(classdata?.className)}
                key={classdata?._id}
